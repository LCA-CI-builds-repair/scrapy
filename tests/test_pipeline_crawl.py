...
class FileDownloadCrawlTestCase(TestCase):
    ...
    def _assert_files_download_failure(self, crawler, items, code, logs):
        ...
        self.assertEqual(
            crawler.stats.get_value("downloader/request_method_count/GET"), expected_get_requests
        )
        self.assertEqual(crawler.stats.get_value("downloader/response_count"), expected_response_count)
        self.assertEqual(
            crawler.stats.get_value("downloader/response_status_count/200"), expected_successful_responses
        )
        self.assertEqual(
            crawler.stats.get_value(f"downloader/response_status_count/{code}"), expected_code_responses
        )
